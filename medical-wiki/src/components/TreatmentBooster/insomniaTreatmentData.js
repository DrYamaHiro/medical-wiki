...
export const MODIFIERS = [
  ...
  { id: 'cm_pruritus_atopic', label: '皮膚そう痒/アトピー誘発（中途覚醒）', cat: '併存疾患' },
  { id: 'cm_gerd_insomnia', label: 'GERD逆流誘発（中途覚醒）', cat: '併存疾患' },
  { id: 'cm_nocturia_insomnia', label: '夜間頻尿誘発（中途覚醒）', cat: '併存疾患' },
  { id: 'cm_copd_orthopnea', label: 'COPD起座呼吸誘発', cat: '併存疾患' },
...
export const RECOMMENDATIONS = [
  ...
  {
    id: 'treat_secondary_cause_first',
    action: 'INVESTIGATE',
    drug: '🎯 二次性不眠: 原疾患を先に治療',
    reason: '原疾患由来の不眠は薬剤よりまず原因治療が GL推奨。原因コントロールで睡眠が改善する例多数',
    fromStates: ['naive', 'transient'],
    preferredWhen: ['cm_pruritus_atopic', 'cm_gerd_insomnia', 'cm_nocturia_insomnia', 'cm_copd_orthopnea', 'cm_pain_related_insomnia'],
    note: '【二次性不眠の原疾患併治療】\n• アトピー/掻痒誘発 → 外用ステロイド/タクロリムス強化、夜間抗ヒスタミン（H1受容体結合の眠気活用）\n• GERD誘発 → PPI/PCAB、就寝3時間前まで食事終了、頭部挙上\n• 夜間頻尿 → α1遮断薬・抗コリン・ループ利尿薬の朝服薬切替\n• COPD起座呼吸 → LABA/LAMA夜間調整、HOT適応評価、酸素飽和度確認\n• 慢性疼痛 → アセトアミノフェン+トラマドール調整、神経障害性ならガバペンチノイド\n→ 原因治療＋必要時オレキシン拮抗薬。BZ/Z は原因放置のまま使うと依存形成しやすい',
  },
  {
