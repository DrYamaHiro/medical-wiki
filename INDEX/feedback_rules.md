...
- **origin に自分が把握していないコミットがある** → **wikiの修正を一旦保留し報告**。他者による修正内容を確認した上で、追記する形で統合するか、改めて書き直すかを検討する（他者の変更を消さない）
- **未コミット変更がある** → 誰かの作業途中の可能性。確認なしで消さない
- **WIKI_EDIT_START/END マーカー** は絶対に削除しない
- 対象ファイルの最終更新を `git log -- <path>` で確認

### 3. 医学監査（多段レビュー）
変更案を作成したら、**必ず複数の独立した監査AIエージェント（最低2、推奨3）**によるレビューを通す。単一エージェントでは指摘漏れが発生することが確認されている（2026-04-13 retroaudit時に確認）。**全エージェントが承認するまで修正→再監査を繰り返す。**

**推奨構成**: 3エージェント並列
1. 臨床妥当性（鑑別の網羅性・マッピング・コメントの正確性）
2. 薬理学・AMS・添付文書照合（用量・禁忌・相互作用）
3. 医療安全・フォーマット（設備制約・帰宅時安全指導・Red Flag severity）
